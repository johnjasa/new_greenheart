...
def overwrite_fin_values(hopp_config):
    ...
    if "pv" in hopp_config["technologies"]:
        if ("pv_om_per_kw" in hopp_config["config"]["cost_info"]) and (
            ...
            for i in range(
                len(
                    hopp_config["technologies"]["pv"]["fin_model"]["system_costs"][
                        "om_capacity"
                    ]
                )
            ):
                hopp_config["technologies"]["pv"]["fin_model"]["system_costs"][
                    "om_capacity"
                ][i] = hopp_config["config"]["cost_info"]["pv_om_per_kw"]
        ...
        if ("pv_om_per_mwh" in hopp_config["config"]["cost_info"]) and (
            ...
            for i in range(
                ...
                hopp_config["technologies"]["pv"]["fin_model"]["system_costs"][
                    "om_production"
                ][i] = hopp_config["config"]["cost_info"]["pv_om_per_mwh"]
            om_pv_variable_cost = hopp_config["technologies"]["pv"]["fin_model"][
                "system_costs"
            ]["om_production"][i]
