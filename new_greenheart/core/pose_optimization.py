...
and also based off of the GreenHeart file of the same name originally adapated by Jared Thomas.
"""
...
class PoseOptimization(object):
    ...
    def get_number_design_variables(self):
        ...
        if "form" in self.config["driver"]["optimization"].keys():
            if self.config["driver"]["optimization"]["form"] == "central": # TODO this should probably be handled at the MPI point to avoid confusion with n_DV being double what would be expected
                n_DV *= 2
    ...
    def _get_step_size(self):
        ...
        if not "step_size" in self.config["driver"]["optimization"]:
            step_size = 1.0E-6
    ...
    def set_driver(self, opt_prob):
        ...
        if self.config["driver"]["optimization"]["flag"]:
            ...
            # The next two optimization methods require pyOptSparse.
                ...
                raise ValueError(f"The {self.config['driver']['optimization']['solver']} optimizer is not yet supported!")
    ...
    def set_objective(self, opt_prob):
        ...
        if self.config.get("objective", False):
            opt_prob.model.add_objective(self.config["objective"]["name"],
    ...
    def set_recorders(self, opt_prob):
        ...
        if self.config["recorder"]["flag"]:
            recorder = om.SqliteRecorder(os.path.join(folder_output, self.config["recorder"]["file_name"]))
